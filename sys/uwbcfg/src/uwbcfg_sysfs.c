...
static ssize_t cfg_show(struct kobject *kobj,
...
static ssize_t cfg_store(struct kobject *kobj,
...
static struct kobj_attribute *dev_attr_cfg = 0;
...
void uwbcfg_sysfs_init(void)
{
    ...
    dev_attr_cfg = kzalloc(num_items * sizeof(struct kobj_attribute), GFP_KERNEL);
    if (!dev_attr_cfg) {
    ...
    for (i=0;i<CFGSTR_MAX;i++) {
        ...
        sysfs_attrs[i] = &dev_attr_cfg[i].attr;
    }
    dev_attr_cfg[CFGSTR_MAX] = DEV_ATTR_ROW("commit");
    sysfs_attrs[CFGSTR_MAX] = &dev_attr_cfg[CFGSTR_MAX].attr;
