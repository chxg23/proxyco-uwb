...
struct uwb_dev_status {
    ...
    uint64_t cca_tx_error:1;          //!< Start tx err because of carrier detected
};
...
struct uwb_mac_interface {
    ...
    bool (* pll_ll_cb)      (struct uwb_dev *, struct uwb_mac_interface *);    //!< PLL Losing Lock callback
 ...
 * @return struct uwb_dev_status
 */
typedef struct uwb_dev_status (*uwb_set_cca_tx_func_t)(struct uwb_dev *dev, bool enable);

/**
 * Check for Carrier Active
 *
 * @param inst    pointer to struct uwb_dev.
 * @return bool   true if carrier active
 */
typedef bool (*uwb_mac_cca_func_t)(struct uwb_dev *dev);
...
struct uwb_driver_funcs {
    ...
    uwb_set_cca_tx_func_t uf_set_cca_tx;
    uwb_mac_cca_func_t uf_mac_cca;
