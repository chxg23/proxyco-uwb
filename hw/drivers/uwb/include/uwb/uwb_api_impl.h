 ...
 */
UWB_API_IMPL_PREFIX struct uwb_dev_status
uwb_set_cca_tx(struct uwb_dev *dev, bool enable)
{
    return (dev->uw_funcs->uf_set_cca_tx(dev, enable));
}
EXPORT_SYMBOL(uwb_set_cca_tx);
 ...
 */
UWB_API_IMPL_PREFIX bool
uwb_mac_cca(struct uwb_dev *dev)
{
    return (dev->uw_funcs->uf_mac_cca(dev));
}
EXPORT_SYMBOL(uwb_mac_cca);
