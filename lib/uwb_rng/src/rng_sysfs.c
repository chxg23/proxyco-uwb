...
static struct rng_sysfs_data* get_instance(struct kobject* kobj)
{
...
static ssize_t rng_show(struct kobject *kobj,
...
static ssize_t rng_store(struct kobject *kobj,
...
int uwbrng_sysfs_init(struct uwb_rng_instance *rng)
{
    ...
    ed->dev_attr_cfg = kzalloc((n_rngtypes)*sizeof(struct kobj_attribute), GFP_KERNEL);
    if (!ed->dev_attr_cfg) {
    ...
    i=0;
    ed->dev_attr_cfg[i] = DEV_ATTR_ROW("listen");
    ed->attrs[i] = &ed->dev_attr_cfg[i].attr;
    i++;
    if(!(SLIST_EMPTY(&rng->rng_configs))){
        SLIST_FOREACH(cfgs, &rng->rng_configs, next){
            ed->dev_attr_cfg[i] = DEV_ATTR_ROW(cfgs->name);
            ed->attrs[i] = &ed->dev_attr_cfg[i].attr;
            i++;
        }
    }
