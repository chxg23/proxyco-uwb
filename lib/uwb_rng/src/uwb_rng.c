...
/**
 * @fn uwb_rng_request_wto(struct uwb_rng_instance * inst, uint16_t dst_address, uwb_dataframe_code_t code, uint32_t timeout)
 ...
 * @param timeout       Timeout for semaphores
 * two way ranging UWB_DATA_CODE_DS_TWR_EXT enables double sided two way ranging with extended frame.
 *
 * @return struct uwb_dev_status
 */
struct uwb_dev_status
uwb_rng_request_wto(struct uwb_rng_instance * rng, uint16_t dst_address, uwb_dataframe_code_t code, uint32_t timeout)
{
    ...
    err = dpl_sem_pend(&rng->sem,  timeout);
    if (err != DPL_OK) {
        goto early_exit;
    }

    /* Prevent any IRQs from interfering */
    err = dpl_sem_pend(&inst->irq_sem,  timeout);
    if (err != DPL_OK) {
    ...
    err = dpl_sem_pend(&rng->sem, timeout); // Wait for completion of transactions
    if (err != DPL_OK) {
        goto early_exit;
    }
